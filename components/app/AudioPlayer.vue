<template>
  <div v-if="playbackSession" id="streamContainer" class="fixed top-0 left-0 layout-wrapper right-0 z-50 pointer-events-none" :class="{ fullscreen: showFullscreen, 'ios-player': $platform === 'ios', 'web-player': $platform === 'web' }">
    <div v-if="showFullscreen" class="w-full h-full z-10 absolute top-0 left-0 pointer-events-auto" :style="{ backgroundColor: coverRgb }">
      ...
      <p class="top-4 absolute left-0 right-0 mx-auto text-center uppercase tracking-widest text-opacity-75" :class="{ 'text-black text-opacity-75': coverBgIsLight }" style="font-size: 10px">{{ isDirectPlayMethod ? $strings.LabelDirectPlayMethod : isLocalPlayMethod ? $strings.LabelLocalPlayMethod : $strings.LabelTranscodePlayMethod }}</p>
    </div>
  ...
  computed: {
    menuItems() {
      ...
      items.push(
        ...[
          {
            text: this.$strings.MenuTotalTrack,
            value: 'total_track',
            icon: this.useTotalTrack ? 'check_box' : 'check_box_outline_blank'
          },
          {
            text: this.$strings.MenuChapterTrack,
            value: 'chapter_track',
            icon: this.useChapterTrack ? 'check_box' : 'check_box_outline_blank'
          },
          {
            text: this.lockUi ? this.$strings.MenuUnlockPlayer : this.$strings.MenuLockPlayer,
            value: 'lock',
            icon: this.lockUi ? 'lock' : 'lock_open'
          },
          {
            text: this.$strings.MenuClosePlayer,
            value: 'close',
            icon: 'close'
          }
        ]

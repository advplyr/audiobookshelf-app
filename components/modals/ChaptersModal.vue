            ...
            <li :key="chapter.id" :id="`chapter-row-${chapter.id}`" class="text-fg select-none relative py-4 cursor-pointer" :class="currentChapterId === chapter.id ? 'bg-primary bg-opacity-80' : ''" role="option" @click="clickedOption(chapter)">
              <div class="relative flex items-center pl-3 pr-20">
                ...
                <div class="absolute top-0 right-3 -mt-0.5">
                  <span v-if="useDurationInChaptersModal" class="font-mono text-fg-muted leading-3 text-sm" style="letter-spacing: -0.5px">{{ $secondsToTimestamp(Math.max(0, chapter.end - chapter.start) / _playbackRate) }}</span>
                  <span v-else class="font-mono text-fg-muted leading-3 text-sm" style="letter-spacing: -0.5px">{{ $secondsToTimestamp(chapter.start / _playbackRate) }}</span>
                </div>
              </div>
...
export default {
  ...
  computed: {
    ...
    useDurationInChaptersModal() {
      return this.$store.getters['getUseDurationInChaptersModal']
    }
  },
  methods: {

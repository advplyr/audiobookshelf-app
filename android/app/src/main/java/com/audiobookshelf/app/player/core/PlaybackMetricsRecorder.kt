...
class PlaybackMetricsRecorder(
  ...
  fun logSummary() {
    try {
      ...
      AbsLogger.info(
        logTag,
        "summary player=$playerId item=$mediaItemId buffers=$bufferCount errors=$playbackErrorCount startupReadyLatencyMs=$timeToFirstReadyMs serviceReadyLatencyMs=$serviceReadyLatencyMs recoverableRetries=$recoverableRetryCount"
      )
    } catch (_: Exception) {
    }
  }
